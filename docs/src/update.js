...
// UPDATE
function updateGrabMouse(go) {
    if (go.grab)
        updateGrab(go);
    if (go.mouse)
        updateMouse(go);
}
export function listGameObject(gos) {
    const res = [];
...
function moveGrabbedCards(grabbedCard, newSlot) {
...
export function findSlotOfCard(card) {
    let slot = card;
    while (slot.stack)
        slot = slot.stack.previous;
    return slot;
}
function findTopCardOfSlot(slot) {
    let slotTop = slot;
    while (true) {
...
function findCardAbove(card) {
    return [...gos.cards.values()].find(go => !!(go.stack && go.stack.previous === card)
...
function isStackingAllowed(bottomCard, topCard) {
